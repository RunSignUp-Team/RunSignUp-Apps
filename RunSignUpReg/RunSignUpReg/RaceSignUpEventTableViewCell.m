...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        self.nameLabel = [[UILabel alloc] initWithFrame: CGRectMake(4, 0, [self frame].size.width - 20, 22)];
        [nameLabel setFont: [UIFont fontWithName:@"OpenSans" size:18]];
        [nameLabel setAdjustsFontSizeToFitWidth: YES];
        [self.contentView addSubview: nameLabel];
        
        self.priceLabel = [[UILabel alloc] initWithFrame: CGRectMake(4, 22, [self frame].size.width - 8, 20)];
        ...
        self.priceLabel2 = [[UILabel alloc] initWithFrame: CGRectMake(4, 22, [self frame].size.width - 8, 20)];
...
- (void)setPrice:(NSString *)price1 price2:(NSString *)price2{
    ...
    if(![price2 isEqualToString:@"$0.00"]){
        ...
        [priceLabel2 setFrame: CGRectMake([priceLabel frame].origin.x + size.width + 4, [priceLabel frame].origin.y, [self frame].size.width - 8 - [priceLabel frame].origin.x + size.width, 20)];
    }else{
...
- (void)setShowPrice:(BOOL)showPrice{
    if(showPrice){
        ...
        [nameLabel setFrame: CGRectMake(4, 0, [[UIScreen mainScreen] bounds].size.width, 20)];
    }else{
        [priceLabel setHidden: YES];
        [priceLabel2 setHidden: YES];
        [nameLabel setFrame: CGRectMake(4, [self.contentView frame].size.height / 2 - 10, [self frame].size.width - 8, 20)];
    }
}
